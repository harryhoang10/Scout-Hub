...
function classifyExtractorError(error: Error & { code?: string }): ErrorCategory {
  ...
  if (code === 'SERVERLESS_CRASH' || /blocked|403|503|unavailable|chặn|phản hồi không hợp lệ/.test(text)) {
    return 'blocked';
  }
...
"""${bio}"""`;
  ...
  const scrapeTikTok = async (url: string, isFastMode: boolean = false, refresh: boolean = false): Promise<Partial<ProfileData>> => {
    ...
    if (!response.ok) {
      let errMsg = 'Lỗi scrape TikTok';
      let errCode = '';
      try {
        const err = await response.json();
        errMsg = err.error || errMsg;
        errCode = err.code || errCode;
      } catch (e) {
        errMsg = 'Serverless Function trả về phản hồi không hợp lệ (HTTP ' + response.status + '). Vui lòng thử dùng Bookmarklet trích xuất lẻ hoặc cấu hình RapidAPI Key.';
        errCode = 'SERVERLESS_CRASH';
      }
      const requestError = new Error(errMsg);
      (requestError as Error & { code?: string }).code = errCode;
      throw requestError;
    }
  ...
  const scrapeFacebook = async (url: string, refresh: boolean = false): Promise<Partial<ProfileData>> => {
    ...
    if (!response.ok) {
      let errMsg = 'Lỗi extract Facebook';
      let errCode = '';
      try {
        const err = await response.json();
        errMsg = err.error || errMsg;
        ...
        errMsg = 'Facebook chặn IP Cloud của Netlify (HTTP ' + response.status + '). Vui lòng kéo và dùng Bookmarklet trích xuất 1-Click để đạt tỉ lệ thành công 100%!';
        errCode = 'SERVERLESS_CRASH';
      }
      const requestError = new Error(errMsg);
      (requestError as Error & { code?: string }).code = errCode;
