...
export function Extractor({ onSaveToRestored }: ExtractorProps) {
      ...
      try {
        ...
        for (const row of data) {
          for (const cell of row) {
            if (typeof cell === 'string' && cell.includes('tiktok.com')) {
              urls.push(cell.trim());
              break;
            }
          }
...
Hãy trả về kết quả dưới dạng JSON với cấu trúc sau:
{
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, "tiktok_extract.xlsx");
  };
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardHeader className="pb-3">
            ...
            <CardDescription>Hệ thống sẽ tự động tìm link TikTok trong file</CardDescription>
          </CardHeader>
